...
export const TransferRoom = () => {
  ...
  useEffect(() => {
    ...
    return () => { 
      socket.off('connect'); socket.off('file-relay'); socket.off('disconnect'); 
             ...
             {p2pState === 'connected' ? (
                ...
                <div className="p-6">
                  ...
                  <div className="relative group">
                    ...
                    <AnimatePresence>
                      {searchQuery && (
                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="absolute top-full left-0 right-0 mt-2 bg-[#1A202C] border border-gray-700 rounded-xl shadow-2xl z-50">
                          {searchResult || (searchQuery.length > 2) ? (
                            <div className="p-3 flex justify-between hover:bg-gray-800 cursor-pointer" onClick={() => sendConnectionRequest(searchQuery)}>
            ...
            {transferStats && (
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-[#121826] border border-gray-800 p-4 rounded-xl flex flex-col justify-between h-24">
                  <div className="flex justify-between text-gray-500 uppercase text-[10px] font-bold"><span>Original</span><Layers className="w-4 h-4" /></div>
                  <span className="text-lg font-bold text-gray-200">{(transferStats.originalSize / 1024 / 1024).toFixed(2)} MB</span>
                </div>
                <div className="bg-[#121826] border border-gray-800 p-4 rounded-xl flex flex-col justify-between h-24">
                  <div className="flex justify-between text-gray-500 uppercase text-[10px] font-bold"><span>Sent</span><Wifi className="w-4 h-4 text-blue-400" /></div>
                  <span className="text-lg font-bold text-gray-200">{(transferStats.finalSize / 1024 / 1024).toFixed(2)} MB</span>
                </div>
                <div className="bg-[#121826] border border-gray-800 p-4 rounded-xl flex flex-col justify-between h-24">
                  <div className="flex justify-between text-gray-500 uppercase text-[10px] font-bold"><span>Speed</span><Activity className="w-4 h-4 text-yellow-400" /></div>
                  <span className="text-lg font-bold text-gray-200">{transferStats.speed} MB/s</span>
                </div>
                <div className="bg-[#121826] border border-gray-800 p-4 rounded-xl flex flex-col justify-between h-24">
                  <div className="flex justify-between text-gray-500 uppercase text-[10px] font-bold"><span>Saved</span><Zap className="w-4 h-4 text-green-400" /></div>
                  <span className="text-lg font-bold text-green-400">{((1 - (transferStats.finalSize / transferStats.originalSize)) * 100).toFixed(0)}%</span>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
            ...
            <div className="flex-1 p-4 font-mono text-xs space-y-1.5 overflow-y-auto custom-scrollbar text-gray-400">
               {logs.map((log, i) => (
                 <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="flex gap-2">
                   ...
                   <span className={clsx(log.includes('CRITICAL') ? "text-red-400" : log.includes('ESTABLISHED') || log.includes('Verified') ? "text-green-400" : "text-gray-300")}>{log}</span>
                 </motion.div>
               ))}
        ...
        {progress > 0 && progress < 100 && (
          <motion.div initial={{ y: 100 }} animate={{ y: 0 }} exit={{ y: 100 }} className="fixed bottom-0 left-0 right-0 bg-[#121826] border-t border-gray-800 p-4 z-50 shadow-2xl">
             <div className="max-w-3xl mx-auto flex items-center gap-4">
               <span className="text-xs font-bold text-blue-400 animate-pulse">TRANSFERRING...</span>
